   ...
   var PDFViewerApplication = {
    ...
    load: function pdfViewLoad(pdfDocument, scale) {
     ...
     firstPagePromise.then(function (pdfPage) {
      ...
      pdfPage.getAnnotations().then(function (annotations) {
        ...
        annotations.forEach(function (annotation) {
          //console.log("------> Raw: "+JSON.stringify(annotation));
          //console.log({title= annotation.title, rect : annotation.rect});
		  if(tempContents == annotation.contents) {
			  // nothing to do
		  } else {
		  if(annotation.contents != " ") {
			  console.log(annotation.contents);
			  pdfContents.push(annotation.contents);
     ...
     pdfDocument.getMetadata().then(function (data) {
      ...
      console.log('PDF ' + pdfDocument.fingerprint + ' [' + info.PDFFormatVersion + ' ' + info.PDFDocumentProperties_parseFileSize + ' ' + (info.Producer || '-').trim() + ' / ' + (info.Creator || '-').trim() + ']' + ' (PDF.js: ' + (pdfjsLib.version || '-') + (!pdfjsLib.PDFJS.disableWebGL ? ' [WebGL]' : '') + ')');
      var pdfTitle;
